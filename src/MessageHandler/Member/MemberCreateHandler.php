...
#[AsMessageHandler]
final class MemberCreateHandler
{
    ...
    public function __invoke(MemberCreate $message): void
    {
        $customer = $this->customerRepository->find($message->customerId);
        if ($customer === null) {
            $this->logger->warning('[Mailchimp] Customer #{id} not found, skipping MemberCreate.', ['id' => $message->customerId]);
        ...
        try {
            ...
            $this->logger->info('[Mailchimp] Member created for customer #{id}.', ['id' => $message->customerId]);
