...
final class OrderCreateHandlerTest extends TestCase
{
    ...
    public function test_upserts_order_and_sets_mailchimp_order_id(): void
    {
        ...
        $this->mailchimpClient->expects(self::once())->method('upsertOrder')->with('web-store', $order, false);
        $this->entityManager->expects(self::once())->method('flush');

        $handler = $this->buildHandler();
        $handler(new OrderCreate(55));

        self::assertSame(IdSanitizer::sanitize('55'), $order->getMailchimpOrderId());
        self::assertNull($order->getMailchimpOrderError());
    }

    public function test_passes_is_in_real_time_flag_to_client(): void
    {
        $channel = new Channel();
        $order = new Order();
        self::setIdOnObject($order, 55);
        ...
        $this->mailchimpClient->expects(self::once())->method('upsertOrder')->with('web-store', $order, true);

        $handler = $this->buildHandler();
        $handler(new OrderCreate(55, isInRealTime: true));
    }
    ...
    public function test_upserts_products_before_order_for_each_unique_product(): void
    {
        ...
        $handler = $this->buildHandler();
        $handler(new OrderCreate(10));
    }

    private function buildHandler(): OrderCreateHandler
    {
