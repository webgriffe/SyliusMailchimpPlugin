...
final class CartUpdateHandler
{
    ...
    public function __invoke(CartUpdate $message): void
    {
        ...
        $this->upsertCartProducts($order, $store->id, $channel, $locale);
        ...
        $this->logger->info('[Mailchimp] Cart updated for order #{id} in store {store}.', ['id' => $message->orderId, 'store' => $store->id]);
    }
