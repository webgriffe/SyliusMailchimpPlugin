    ...
    public function getConfigTreeBuilder(): TreeBuilder
    {
        ...
        $rootNode
            ->children()
                ...
                ->end()
                ->scalarNode('webhook_signing_secret')
                    ->defaultValue('')
                    ->info('Signing secret to verify the HMAC signature ("X-Mailchimp-Signature" header) of incoming Mailchimp webhooks; empty disables signature verification')
                ->end()
                ->booleanNode('send_unpaid_orders_as_carts')
                    ->defaultTrue()
                    ->info('Send unpaid/pending orders to Mailchimp as abandoned carts')
                ->end()
                ->booleanNode('command_lock_enable')
                    ->defaultTrue()
                    ->info('Enable Symfony Lock for CLI commands to prevent concurrent execution')
                ->end()
                ->arrayNode('mapper')
                    ->addDefaultsIfNotSet()
                    ->children()
